...
import numpy as np
import ase.calculators.nwchem as acn

from .calculator import C2TKFileIOCalculator
from .. import is_mpi_enabled, settings


class NWChem(C2TKFileIOCalculator, acn.NWChem):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        if is_mpi_enabled():
            self.command = 'nwchem_openmpi PREFIX.nwi >> PREFIX.nwo 2> PREFIX.err'
        else:
            self.command = 'nwchem PREFIX.nwi >> PREFIX.nwo 2> PREFIX.err'
